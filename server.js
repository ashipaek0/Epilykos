...
function initializeDatabase() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS history (
      ...
      consumption REAL, solar REAL, battery_charge REAL, battery_discharge REAL,
      grid_import REAL, grid_export REAL, battery_soc REAL,
      daily_consumption REAL, daily_solar REAL, daily_battery_charge REAL,
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS metrics (
      timestamp INTEGER NOT NULL, metric TEXT NOT NULL, value REAL,
      PRIMARY KEY (timestamp, metric)
    );
    CREATE TABLE IF NOT EXISTS latest_metrics (
      metric TEXT PRIMARY KEY, value REAL, timestamp INTEGER
    );
  `);
...
function migrateLegacyConfig() {
  ...
  if (!haDevicesStr || JSON.parse(haDevicesStr || '[]').length === 0) {
    ...
    if (haUrl || haToken) {
      const entities = {};
      ['consumption','solar','battery_charge','battery_discharge','grid_import','grid_export','battery_soc',
       'daily_consumption','daily_solar','daily_battery_charge','daily_battery_discharge',
       'daily_grid_import','daily_grid_export','battery_voltage','inverter_temp','solar_voltage','load_power']
      .forEach(m => { const e = getConfig(`ha_entity_${m}`); if (e) entities[m] = e; });
      const device = { name:'Home Assistant', url:haUrl, token:haToken, enabled:haEnabled, poll_interval:30, entities };
  ...
  if (!mqttDevicesStr || JSON.parse(mqttDevicesStr || '[]').length === 0) {
    ...
    if (brokerUrl) {
      ...
      ['consumption','solar','battery_charge','battery_discharge','grid_import','grid_export','battery_soc',
       'daily_consumption','daily_solar','daily_battery_charge','daily_battery_discharge',
       'daily_grid_import','daily_grid_export','battery_voltage','inverter_temp','solar_voltage','load_power']
      .forEach(m => { const t = getConfig(`mqtt_topic_${m}`); if (t) topics[m] = t; });
      const device = { name:'MQTT Broker', broker:brokerUrl, username:getConfig('mqtt_username'), password:getConfig('mqtt_password'), enabled:getConfig('mqtt_enabled')==='true', topics };
...
function parseGridState(state) {
  ...
  if (['on','true','1','open','unlocked'].includes(str)) return 1;
  return 0;
}
...
app.get('/api/monthly', async (req, res) => { /* same as before */ });

app.get('/api/grid/status', async (req, res) => { /* same as before */ });

app.get('/api/grid/hours', async (req, res) => { /* same as before */ });

app.get('/api/grid/timeline', async (req, res) => { /* same as before */ });

app.get('/api/savings', async (req, res) => { /* same as before */ });

app.get('/api/solar-forecast', async (req, res) => { /* same as before */ });

app.get('/api/dashboard-state', async (req, res) => { /* same as before */ });

app.get('/api/metrics/names', async (req, res) => { /* same as before */ });

// ── Authenticated routes ──────────────────────
app.get('/api/test-forecast', authMiddleware, async (req, res) => { /* same as before */ });
...
app.post('/api/dashboard-config', authMiddleware, async (req, res) => {
  try {
    setConfig('dashboard_config', JSON.stringify(req.body));
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/test-mqtt', authMiddleware, async (req, res) => { /* same as before */ });
app.get('/api/test-mqtt-topic', authMiddleware, async (req, res) => { /* same as before */ });
...
app.post('/api/restore', authMiddleware, upload.single('dbfile'), async (req, res) => { /* same as before */ });
...
app.post('/api/settings', async (req, res) => { /* same as before */ });

app.get('/api/modbus/profiles', authMiddleware, (req, res) => {
  res.json(availableProfiles.map(p => ({ id: p.id, name: p.name })));
});
app.get('/api/test-modbus', authMiddleware, async (req, res) => { /* same as before */ });

app.get('/settings', authMiddleware, (req, res) => res.sendFile(path.join(__dirname, 'public', 'settings.html')));

app.get('/api/metrics/current', async (req, res) => { /* same as before */ });
app.get('/api/metrics/history', async (req, res) => { /* same as before */ });
