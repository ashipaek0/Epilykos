...
function normalizeDatasets(datasets) {
  if (!datasets || !datasets.length) return [{ label: 'Solar Generated', metric: 'daily_solar', color: '#FFEA00' }, { label: 'Grid Imported', metric: 'daily_grid_import', color: '#FF4255' }, { label: 'Energy Consumed', metric: 'daily_consumption', color: '#0062FF' }];
  if (typeof datasets[0] === 'string') { const lm = { solar: { label: 'Solar Generated', color: '#FFEA00' }, grid_import: { label: 'Grid Imported', color: '#FF4255' }, consumption: { label: 'Energy Consumed', color: '#0062FF' }, battery_charge: { label: 'Battery Charge', color: '#00E056' }, battery_discharge: { label: 'Battery Discharge', color: '#00E056' }, grid_export: { label: 'Grid Export', color: '#f59e0b' } }; return datasets.map(ds => { const b = lm[ds] || { label: ds, color: '#888' }; return { label: b.label, metric: ds, color: b.color }; }); }
