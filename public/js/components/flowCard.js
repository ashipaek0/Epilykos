...
export function updateFlowCard(state) {
  document.querySelectorAll('.flow-card').forEach(card => {
    ...
    const gn = gi - ge, gDir = gn >= 0 ? 'Import' : 'Export', gCol = gn >= 0 ? 'var(--grid)' : '#3b82f6';
    ...
    [['icon-solar', sw > 0 ? 'var(--solar)' : 'var(--text)'], ['icon-home', cw > 0 ? 'var(--home)' : 'var(--text)'], ['icon-grid', gn > 0 ? 'var(--grid)' : gn < 0 ? '#3b82f6' : 'var(--text)']].forEach(([k, v]) => { const e = el(k); if (e) e.style.color = v; });
    ...
    const ga = card.querySelector('.flow-arrow.grid'); if (ga) { if (gi > ge) { ga.style.color = 'var(--grid)'; ga.textContent = '←'; } else if (ge > gi) { ga.style.color = '#3b82f6'; ga.textContent = '→'; } else { ga.style.color = 'var(--text-secondary)'; ga.textContent = '⇄'; } }
    const gtb = card.querySelector(`#${uid('grid-to-battery',id)}`); if (gtb) gtb.style.display = isGridChargingBattery ? 'block' : 'none';
