  ...
  switch (type) {
    ...
    case 'bar-gauge': {
      ...
      for (var j = 0; j < bgRows.length; j++) {
        var labelEl = document.querySelector('.bg-label:nth-of-type(' + (j+1) + ')');
        if (labelEl) bgRows[j].label = labelEl.value;
        var msel2 = document.getElementById('bg-metric-' + j);
        if (msel2) bgRows[j].metric = msel2.value;
        var uel = document.querySelector('.bg-unit:nth-of-type(' + (j+1) + ')');
        if (uel) bgRows[j].unit = uel.value;
        var minel = document.querySelector('.bg-min:nth-of-type(' + (j+1) + ')');
        if (minel) bgRows[j].min = parseFloat(minel.value) || 0;
        var maxel = document.querySelector('.bg-max:nth-of-type(' + (j+1) + ')');
        if (maxel) bgRows[j].max = parseFloat(maxel.value) || 100;
        var cel = document.querySelector('.bg-color:nth-of-type(' + (j+1) + ')');
        if (cel) bgRows[j].color = cel.value;
        var gel = document.querySelector('.bg-gradient:nth-of-type(' + (j+1) + ')');
    ...
    case 'bar-gauge-retro': {
      ...
      for (var k = 0; k < bgrRows.length; k++) {
        var labelEl2 = document.querySelector('.bgr-label:nth-of-type(' + (k+1) + ')');
        ...
        var uel2 = document.querySelector('.bgr-unit:nth-of-type(' + (k+1) + ')');
        if (uel2) bgrRows[k].unit = uel2.value;
        var minel2 = document.querySelector('.bgr-min:nth-of-type(' + (k+1) + ')');
        if (minel2) bgrRows[k].min = parseFloat(minel2.value) || 0;
        var maxel2 = document.querySelector('.bgr-max:nth-of-type(' + (k+1) + ')');
        if (maxel2) bgrRows[k].max = parseFloat(maxel2.value) || 100;
        var selEl = document.querySelector('.bgr-segments:nth-of-type(' + (k+1) + ')');
        if (selEl) bgrRows[k].segments = parseInt(selEl.value) || 10;
        var cel2 = document.querySelector('.bgr-color:nth-of-type(' + (k+1) + ')');
        if (cel2) bgrRows[k].color = cel2.value;
        var gel2 = document.querySelector('.bgr-gradient:nth-of-type(' + (k+1) + ')');
    ...
    case 'metric-cards': {
      ...
      for (var m = 0; m < mcRows.length; m++) {
        var tEl = document.querySelector('.mc-title:nth-of-type(' + (m+1) + ')');
        if (tEl) mcRows[m].title = tEl.value;
        var mmel = document.getElementById('mc-metric-' + m);
        if (mmel) mcRows[m].metric = mmel.value;
        var uel3 = document.querySelector('.mc-unit:nth-of-type(' + (m+1) + ')');
    ...
    case 'chart-energy': {
      ...
      for (var c = 0; c < chRows.length; c++) {
        var lEl = document.querySelector('.chart-label:nth-of-type(' + (c+1) + ')');
        if (lEl) chRows[c].label = lEl.value;
        var cmel = document.getElementById('chart-metric-' + c);
        if (cmel) chRows[c].metric = cmel.value;
        var ccel = document.querySelector('.chart-color:nth-of-type(' + (c+1) + ')');
