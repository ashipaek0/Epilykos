 ...
 *
 * Fixtures marked REAL were captured from the live GETA dongle
 * (LXP0000001 / LXP0000002 @ 192.168.0.23:8000) on 2026-09-05
 * (/home/ubuntu/epilykos-tracker/luxpower-geta-discovery.json, crcOk:true) —
 * they are genuine wire response frames, not synthesized.
 *
...
const AC2_HEX = 'a11a0500200001c24c585030303030303031120000044c58503030303030303200002800b019';
const AC2 = Buffer.from(AC2_HEX, 'hex');

// REAL: input registers 0-39 response (devFn 0x04, start 0, byte_len 0x50)
const REAL_INPUT_0_39 = Buffer.from('a11a05006f0001c24c585030303030303031610001044c585030303030303032000050140036090000000012015e000000ae010000000000000000690800008138a51358010000aa0000006b0810325c10a6130000000000004f023600000000001c002700270026003000000052008a10fb0ea67a', 'hex');

// REAL: input registers 40-79 response (devFn 0x04, start 40, byte_len 0x50)
const REAL_INPUT_40_79 = Buffer.from('a11a05006f0001c24c585030303030303031610001044c5850303030303030322800503600000000000000000000001c00000027000000270000002600000030000000000000005200000000000000000000002b002700310000000000ea2a0100000000000000000000000000200000000000c6f7', 'hex');

// REAL: input registers 80-119 response (devFn 0x04, start 80, byte_len 0x50)
const REAL_INPUT_80_119 = Buffer.from('a11a05006f0001c24c585030303030303031610001044c5850303030303030325000501104cb01e80312019600000000000000000000007132bfab0e03f0606d1003000000b301000000000000000000000000000012010000120100000000000000000000040100004c585030303030303032827d', 'hex');

// REAL: holding registers 80-119 response (devFn 0x03, start 80, byte_len 0x50)
const REAL_HOLDING_80_119 = Buffer.from('a11a05006f0001c24c585030303030303031610001034c5850303030303030325000500000000000000000000000000000000000000000e600320000000000000000000000000000001c01d20064006400000000000f0038ff260200009001810c000000000100000000000000000000001400c9f8', 'hex');

// REAL: unsolicited holding 0-79 push (devFn 0x03, start 0, byte_len 0xA0)
const REAL_UNSOL_HOLDING_0_79 = Buffer.from('', 'hex');

// ---------------------------------------------------------------------------
// AC1: CRC16/Modbus over the known-good request inner = 0x19B0 (wire b0 19)
// ---------------------------------------------------------------------------
{
  const inner = AC2.slice(20, AC2.length - 2);
  assert.strictEqual(crc16Modbus(inner), 0x19B0, 'AC1: CRC over request inner must be 0x19B0');
  assert.strictEqual(AC2[AC2.length - 2], 0xB0, 'AC1: CRC stored low byte must be 0xB0');
  assert.strictEqual(AC2[AC2.length - 1], 0x19, 'AC1: CRC stored high byte must be 0x19');
  console.log('PASS AC1: crc16Modbus(request inner) = 0x' + crc16Modbus(inner).toString(16) + ' (wire b0 19)');
}
