...
async function initEditor() {
  ...
  try {
    ...
    document.getElementById('save-btn').addEventListener('click', async () => { await persistLayout(); clearUnsaved(); const validTab = dashboardConfig.dashboards.find(db => db.id === currentTabId) ? currentTabId : dashboardConfig.dashboards[0]?.id || 'main'; window.location.href = '/?tab=' + validTab; });
    hideLoading();
